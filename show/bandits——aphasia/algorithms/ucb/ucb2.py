...
class UCB2(object):
  def __init__(self, alpha, counts, values,current_arm,next_update,r):
    """
  ...
  def __set_arm(self, arm):
    ...
    self.next_update += max(1, self.__tau(self.r[arm] + 1) - self.__tau(self.r[arm]))
    self.r[arm] += 1
  ...
  def select_arm(self):
    ...
    if self.next_update > sum(self.counts):
